...
export default function Page({ params }: { params: { slug: string } }) { const entry = industries.find(x => x.slug === params.slug); if (!entry) notFound(); return <><BreadcrumbSchema items={[{ name: "Home", path: "/" }, { name: "Industries", path: "/industries" }, { name: entry.title, path: `/industries/${entry.slug}` }]} /><SeoLandingPage entry={entry} backHref="/industries" backLabel="All industries" /></>; }
