...
export default function Page({ params }: { params: { slug: string } }) { const entry = technologies.find(x => x.slug === params.slug); if (!entry) notFound(); return <><BreadcrumbSchema items={[{ name: "Home", path: "/" }, { name: "Technologies", path: "/technologies" }, { name: entry.title, path: `/technologies/${entry.slug}` }]} /><SeoLandingPage entry={entry} backHref="/technologies" backLabel="All technologies" /></>; }
